SamplerState gInputImageSampler : register (s0);
...
float4 PS(VSOutput pIn) : SV_Target
{
	...
	float4 color = cWeights[5] * gInputImage.SampleLevel(gInputImageSampler, pIn.texCoord, 0.0f);
	float totalWeight = cWeights[5];

	float4 centerNormalDepth = gNormalDepthMap.SampleLevel(gNormalDepthSampler, pIn.texCoord, 0.0f);
	...
	for(int i = -cBlurRadius; i <= cBlurRadius; ++i)
	{
		...
		float4 neighborNormalDepth = gNormalDepthMap.SampleLevel(gNormalDepthSampler, tex, 0.0f);
			...
			abs(neighborNormalDepth.a - centerNormalDepth.a) <= 0.2f)
		{
			...
			color += weight * gInputImage.SampleLevel(gInputImageSampler, tex, 0.0f);
