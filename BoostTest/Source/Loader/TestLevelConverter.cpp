...
BOOST_AUTO_TEST_CASE(TestCreateLevel)
{
	...
	modelInfo.at(1).m_MeshName = "Bins";
	modelInfo.at(1).m_Animated = false;
	modelInfo.at(1).m_Transparency = false;
	modelInfo.at(1).m_Collidable = true;
	...
	BOOST_CHECK_EQUAL_COLLECTIONS(resHeader.begin() + 124, resHeader.begin() + 128, intTrue.c, intTrue.c + sizeof(int));
	conv.clear();
}
