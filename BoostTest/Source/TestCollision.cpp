...
BOOST_AUTO_TEST_CASE(SphereVsSphereHit)
{
	...
	BOOST_CHECK_EQUAL(col.sphereVsSphere(&s1, &s2), true);
}
...
BOOST_AUTO_TEST_CASE(SphereVsSphereMiss)
{
	...
	BOOST_CHECK_EQUAL(col.sphereVsSphere(&s1, &s2), false);
}
...
BOOST_AUTO_TEST_CASE(AABBVsSphereHit)
{
	...
	BOOST_CHECK_EQUAL(col.AABBvsSphere(&aabb, &s1), true);
}
...
BOOST_AUTO_TEST_CASE(AABBVsSphereMiss)
{
	...
	BOOST_CHECK_EQUAL(col.AABBvsSphere(&aabb, &s1), false);
}
...
BOOST_AUTO_TEST_CASE(AABBvsAABBHit)
{
	...
	BOOST_CHECK_EQUAL(col.AABBvsAABB(&aabb, &aabbb), true);
}
...
BOOST_AUTO_TEST_CASE(AABBvsAABBMiss)
{
	...
	BOOST_CHECK_EQUAL(col.AABBvsAABB(&aabb, &aabbb), false);
}
