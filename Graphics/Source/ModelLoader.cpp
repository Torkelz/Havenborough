			...
			for(int i = 0; i < m_NumberOfMaterials; i++)
			{
				...
				while(std::getline(input,line))
				{
					...
					for(int i = 0; i < atoi(key.c_str()); i++)
					{
						...
						tempFace.m_Vertex.push_back(tempInt);
						stringstream >> tempInt >> filler;
						tempFace.m_Tangents.push_back(tempInt);
						stringstream >> tempInt >> filler;
						tempFace.m_Normals.push_back(tempInt);
						stringstream >> tempInt >> filler;	
						tempFace.m_TextureCoord.push_back(tempInt);
					}
...
std::vector<DirectX::XMFLOAT3> *ModelLoader::getNormal()
{
	return &m_Normals;
}
std::vector<DirectX::XMFLOAT3> *ModelLoader::getTangent()
{
	return &m_Tangents;
}
std::vector<DirectX::XMFLOAT2> *ModelLoader::getUV()
{
